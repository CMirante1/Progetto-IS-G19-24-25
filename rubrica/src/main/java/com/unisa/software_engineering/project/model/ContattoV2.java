...
public class ContattoV2 implements Comparable<ContattoV2>, Serializable {
    ...
    private static final int NUM_DATI = 3;
    ...
    */
    ...
    private void verificaDati(String nome, String cognome, String[] numeri, String[] emails)throws InfoContattoException{
        ...
        for(int i = 0; i < NUM_DATI; i++){
            if ((numeri[i] != null && !numeri[i].matches("\\d+")))
                throw new InfoContattoException("I numeri telefonici possono contenere soltanto numeri.");
            if ((emails[i] != null && !emails[i].matches("^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$")))
                throw new InfoContattoException("Gli indirizzi email devono contenere una @ e un dominio.");
        }
    }
        /**
     * 
